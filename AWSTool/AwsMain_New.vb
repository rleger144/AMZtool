...
Public Class AwsMain_New
    ...
    Dim glbWathListDT As DataTable
    ...
    Dim HDtasks_SingeMailAtScan As New List(Of Task(Of Int32))()
    Dim columnfilepath As String = Application.StartupPath & "\columnsetting.xml"
    Dim glbNextSchedule As DateTime = Date.Now
    Dim _gblSentMailRecords As New List(Of String)()

    Public AwsCls As New AWSToolClass
    ...
    Private Sub mnuStart_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuStart.ItemClick
        ...
        lblProcessType.Text = "Updating Price (Auto)"
    ...
    Private Sub _AddToWatch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles repoAddtoWatch.Click
        Try
            Dim proid As Integer = objval.ToInt32(dgvGrid.GetRowCellValue(dgvGrid.FocusedRowHandle, "product_id"))
            Dim state As Boolean = objval.ToBoolean(dgvGrid.GetRowCellValue(dgvGrid.FocusedRowHandle, "is_watch"))

            Dim cmd As New SQLiteCommand
            cmd.CommandText = "update db_products set is_watch=@is_watch where product_id=@product_id"
            cmd.Parameters.AddWithValue("@product_id", proid)
            cmd.Parameters.AddWithValue("@is_watch", IIf(state = True, False, True))
            If dbOperation.ExecuteQuery(cmd) > 0 Then
                Dim explog As Int32 = 0
                glbWathListDT = dbOperation.GetDataTable("select product_asin from db_products where is_active='Yes' and is_watch=1", explog)
            End If
        Catch ex As Exception
            ...
            If dr.Length > 0 Then
                ...
                If showAlert = True Then
                    If GblShowNotification = True Then
                        If glbWathListDT.Select("product_asin='" & asin & "'").Length > 0 Then
                            AlertControl1.Show(Me, "<b>" & title & "</b>", asin & Nprice & Uprice, asin & Nprice & Uprice, Nothing, New NotifierData() With {.IsWarning = False}, Color.FromArgb(204, 255, 204))
                        Else
                            If GblShowNotification_type = 0 Then
                                hdAlert.Show(Me, "<b>" & title & "</b>", asin & Nprice & Uprice)
                            End If
                        End If

                        'hdAlert.Show(Me, "<b>" & title & "</b>", asin & Nprice & Uprice)
                        'AlertControl1.Show(Me, "<b>" & title & "</b>", asin & Nprice & Uprice, "Hot-tracked text", Nothing, New NotifierData() With {.IsWarning = False}, Color.White)
                    End If
                    ...
                    If upc.Trim <> "" AndAlso GblEnabledEbay = True Then
                        HDtasks.Add(Task.Factory.StartNew(Function() Me.CompletedsITEM(upc, asin, title)))
                    End If
    ...
    Public Function SendMail_SingleScan(ByVal asinCode As String, ByVal itemTitle As String) As Int32
        Dim isSucc As Int32 = 1
        ...
        Try
            ...
            Mail.Subject = "Amazon Item Alert: ASIN:" & asinCode & " ," & itemTitle & " Date:" & sendDate
            ...
            Dim qry As String = "select *, round( (case when target_price>0 and live_newprice>0 then ((live_newprice/target_price)*100)-100  else 0.00 end),2) as new_per," &
                                ...
                                " from db_products " &
                                " where product_asin in ('" & asinCode & "')"
            ...
            If exLog = 1 Then
                ...
                For Each dr As DataRow In DtMail.Rows
                    mailStr.AppendLine("<tr " & IIf(objval.ToString(dr("is_watch")) = "1", "style='background-color:#c8f1a8;'", "") & "><td width='380' style='font-size:12px;'>" & objval.ToString(dr("product_name")) & "</td>" &
                                           "<td width='70' style='background-color:#ddd;'></td>" &
                                            "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Amazon $</td> " &
                                            "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Target $</td>" & "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Am.Diff%</td>" &
                                            "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Ebay$ Lwst</td>" &
                                            "<td width='80' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Ebay$ CompHigh</td>" & "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Ebay.Avg%</td>" & "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>TradeIN</td>" &
                                            "<td style='background-color:#ddd;font-size:12px;font-weight:bold;'>Rank</td></tr>")

                    mailStr.AppendLine("<tr " & IIf(objval.ToString(dr("is_watch")) = "1", "style='background-color:#c8f1a8;'", "") & "><td style='font-weight:bold;'>" & objval.ToString(dr("product_category")) & "</td>" &
                                           "<td style='background-color:#ddd;font-size:12px;font-weight:bold;'>New Price</td>" &
                                           "<td style='background-color:#f9eee7;'>$" & objval.ToString(dr("live_newprice")) & "</td>" &
                                           "<td " & IIf(objval.ToInt32(dr("isNpriceHigh")) = 1, "style='font-weight:bold;color:#f21717;'", "") & ">$" & objval.ToString(dr("target_price")) & "</td>" & "<td >" & objval.ToString(dr("new_per")) & "%</td>" &
                                           "<td style='background-color:#e2f9ef;'>$" & objval.ToString(dr("ebay_live_new_price")) & "</td>" &
                                           "<td>" & GetEbayPriceSplit(objval.ToString(dr("ebay_sold_new_price_high_multi"))) & "</td>" & "<td>" & objval.ToString(dr("ebay_new_comprice_per")) & "%</td>" & "<td>" & IIf(objval.ToInt32(dr("trade_in_price")) > 0, "$" & objval.ToString(dr("trade_in_price")), "") & "</td>" &
                                           "<td>" & objval.ToString(dr("rank")) & "</td></tr>")

                    mailStr.AppendLine("<tr " & IIf(objval.ToString(dr("is_watch")) = "1", "style='background-color:#c8f1a8;'", "") & "><td><b>ASIN | UPC : </b> " & objval.ToString(dr("product_asin")) & " | " & objval.ToString(dr("upc")) & "</td> " &
                                           "<td style='background-color:#ddd;font-size:12px;font-weight:bold;'>Used Price</td>" &
                                           "<td style='background-color:#f9eee7;'>$" & objval.ToString(dr("live_usedprice")) & "</td>" &
                                           "<td " & IIf(objval.ToInt32(dr("isUpriceHigh")) = 1, "style='font-weight:bold;color:#f21717;'", "") & ">$" & objval.ToString(dr("target_used_price")) & "</td>" & "<td>" & objval.ToString(dr("used_per")) & "%</td>" &
                                           "<td style='background-color:#e2f9ef;'>$" & objval.ToString(dr("ebay_live_used_price")) & "</td><td>" & GetEbayPriceSplit(objval.ToString(dr("ebay_sold_used_price_high_multi"))) & "</td>" & "<td></td>" & "<td>" & objval.ToString(dr("ebay_new_comprice_per")) & "%</td>" &
                                           "<td style='font-size:11px;'><a href='" & objval.ToString(dr("product_url")) & "'>Go to Amazon</a> | <a href='https://www.amazon.com/gp/offer-listing/" & objval.ToString(dr("product_asin")) & "/'> Offer Listing</a> |")
                    ...
                    mailStr.AppendLine("<a href='http://www.ebay.com/sch/i.html?_nkw=" & objval.ToString(dr("upc")) & "'>Ebay Active List</a></td></tr>")
    ...
    Public Function CompletedsITEM(ByVal UPCcode As String, ByVal ASIN As String, ByVal _title As String) As Int64
        ...
        Try
            ...
            If _GblSendMailonEachscan = True Then
                If _gblSentMailRecords.Contains(ASIN) = False Then
                    _gblSentMailRecords.Add(ASIN)
                    HDtasks_SingeMailAtScan.Add(Task.Factory.StartNew(Function() Me.SendMail_SingleScan(ASIN, _title)))
                End If
            End If
    ...
    Private Sub HDbackWork_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles HDbackWork.DoWork
        ...
        Try
            ...
            Dim exLog As Integer = 0
            glbProcessDT = dbOperation.GetDataTable("select product_asin,target_price,target_used_price,upc,notes,is_watch from db_products where is_active='Yes'", exLog)
            _totalProcessCnt = 0
            _totalProcessSucc = 0
...
ProcessCompleted:
                ...
                If exLog = 1 Then
                    ...
                    For Each dr As DataRow In DtMail.Rows
                        mailStr.AppendLine("<tr><td width='380' style='font-size:12px;'>" & objval.ToString(dr("product_name")) & "</td><td width='70' style='background-color:#ddd;'></td>" &
                                            ...
                                            "<td width='80' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Ebay.Comp$</td>" & "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>Ebay.Avg%</td>" & "<td width='60' style='background-color:#ddd;font-size:12px;font-weight:bold;'>TradeIN</td>" &
                                            "<td style='background-color:#ddd;font-size:12px;font-weight:bold;'>Rank</td></tr>")
                        ...
                        mailStr.AppendLine("<tr><td style='font-weight:bold;'>" & objval.ToString(dr("product_category")) & "</td><td style='background-color:#ddd;font-size:12px;font-weight:bold;'>New Price</td>" &
                                           ...
                                           "<td " & IIf(objval.ToInt32(dr("isNpriceHigh")) = 1, "style='font-weight:bold;color:#f21717;'", "") & ">$" & objval.ToString(dr("target_price")) & "</td>" & "<td >" & objval.ToString(dr("new_per")) & "%</td>" &
                                           "<td style='background-color:#e2f9ef;'>$" & objval.ToString(dr("ebay_live_new_price")) & "</td>" &
                                           "<td>$" & objval.ToString(dr("ebay_new_comprice_avg")) & "</td>" & "<td>" & objval.ToString(dr("ebay_new_comprice_per")) & "%</td>" & "<td>" & IIf(objval.ToInt32(dr("trade_in_price")) > 0, "$" & objval.ToString(dr("trade_in_price")), "") & "</td>" &
                                           "<td>" & objval.ToString(dr("rank")) & "</td></tr>")
                        ...
                        mailStr.AppendLine("<tr><td><b>ASIN | UPC : </b> " & objval.ToString(dr("product_asin")) & " | " & objval.ToString(dr("upc")) & "</td> " &
                                           ...
                                           "<td>" & objval.ToString(dr("used_per")) & "%</td><td style='background-color:#e2f9ef;'>$" & objval.ToString(dr("ebay_live_used_price")) & "</td><td>$" & objval.ToString(dr("ebay_used_comprice_avg")) & "</td><td>" & objval.ToString(dr("ebay_new_comprice_per")) & "%</td>" &
                                           "<td></td>" &
    ...
    Private Sub mnuUpdate_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuUpdate.ItemClick
        ...
        mnuStart.Enabled = False
        mnuStop.Enabled = True
        lblProcessType.Text = "Updating Price (Manual)"
        If mnuAWSScan.Checked = True Then
            If HDbackWork.IsBusy = False Then
                HDbackWork.RunWorkerAsync()
            End If
...
#Region "SERVICE Sync Code"
    ...
    Public Function GetAWSItem(AWSSellerID As [String], AWSAccessKey As [String], AWSSecretKey As [String], ASINNo As [String], ByRef _price As [Double], ByRef _newprice As [Double],
...
#End Region
    ...
    Private Sub hdAlert_BeforeFormShow(sender As Object, e As DevExpress.XtraBars.Alerter.AlertFormEventArgs) Handles hdAlert.BeforeFormShow, AlertControl1.BeforeFormShow
        e.AlertForm.OpacityLevel = 1
    End Sub

    Private Sub hdAlert_AlertClick(sender As Object, e As DevExpress.XtraBars.Alerter.AlertClickEventArgs) Handles hdAlert.AlertClick, AlertControl1.AlertClick
        Try
            Dim lnk As String = e.Info.Text.Trim
            Dim arr() As String = lnk.Split(New String() {Environment.NewLine}, StringSplitOptions.None)
            If arr.Length > 0 Then
                Process.Start("https://www.amazon.com/dp/" & objval.ToString(arr(0)).Replace("'", ""))
            End If
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub MWSBackWork_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles MWSBackWork.DoWork
        ...
        Try
            glbMailDT.Rows.Clear()
            Dim exLog As Integer = 0
            glbProcessDT = dbOperation.GetDataTable("select product_asin,target_price,target_used_price,upc,notes,product_name,is_watch from db_products  where is_active='Yes'", exLog)
...
ProcessCompleted:
    ...
    Private Sub MWSBackWork_RunWorkerCompleted(sender As Object, e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles MWSBackWork.RunWorkerCompleted
        ...
        If glbMailDT.Rows.Count > 0 And _GblSenderMailID.Length > 0 And GblIsAlertMail = True And _GblSendMailonEachscan = False Then
            SplashScreenManager.ShowForm(GetType(ozLoading))
            SplashScreenManager.Default.SetWaitFormDescription("Sending Mail..")
            Dim mailaddr As String = GblAlertMailIDs
            Dim SmtpServer As New SmtpClient()
                ...
                If exLog = 1 Then
                    ...
                    For Each dr As DataRow In DtMail.Rows
                        ...
                        mailStr.AppendLine("<tr " & IIf(objval.ToString(dr("is_watch")) = "1", "style='background-color:#c8f1a8;'", "") & "><td style='font-weight:bold;'>" & objval.ToString(dr("product_category")) & "</td>" &
                                           "<td style='background-color:#ddd;font-size:12px;font-weight:bold;'>New Price</td>" &
                                           "<td style='background-color:#f9eee7;'>$" & objval.ToString(dr("live_newprice")) & "</td>" &
                                           "<td " & IIf(objval.ToInt32(dr("isNpriceHigh")) = 1, "style='font-weight:bold;color:#f21717;'", "") & ">$" & objval.ToString(dr("target_price")) & "</td>" & "<td >" & objval.ToString(dr("new_per")) & "%</td>" & "<td style='background-color:#e2f9ef;'>$" & objval.ToString(dr("ebay_live_new_price")) & "</td>" &
                                           "<td>" & GetEbayPriceSplit(objval.ToString(dr("ebay_sold_new_price_high_multi"))) & "</td>" & "<td>" & objval.ToString(dr("ebay_new_comprice_per")) & "%</td>" & "<td>" & IIf(objval.ToInt32(dr("trade_in_price")) > 0, "$" & objval.ToString(dr("trade_in_price")), "") & "</td>" &
                                           "<td>" & objval.ToString(dr("rank")) & "</td></tr>")
        ...
        'HDtasks_SingeMailAtScan
        If _GblSendMailonEachscan = True Then
chkSinglTread:
            Dim taskCnt As Int64 = HDtasks_SingeMailAtScan.Where(Function(x) x.Status = TaskStatus.Running).ToList().Count()
        ...
        Try
            If Convert.ToString(glbErroLog).Length > 0 Then
    ...
    Public Function GetEbayPriceSplit(ByVal _value As String) As String
        If _value = "" Then
            Return ""
        Else
            Dim arr() As String = _value.Split("|")
            If arr.Length > 0 Then
                Return arr(0).Trim()
            Else
                Return ""
...
NextLotofItems:
                ...
                'Dim DtRead As DataTable = dbOperation.GetDataTable("select product_id,product_asin from db_products where product_id >" & _lastIndex & " order by product_id;", exLog)
...
ProcessCompleted:
    ...
    Public Sub setInWatchList(ByVal AddStatus As Boolean)
        Dim arr As String() = GetStockSelection()
        Dim itemsStr As String = String.Join(",", arr)
        If itemsStr.Trim.Length = 0 Then
            objval.MsgWarning("Please select item first.", "Selection Required")
            Exit Sub
        End If

        If AddStatus = True Then
            If objval.MsgConfirmYesNo("Are you sure you want to set selected Items in Watchlist?", "Confirm") = Windows.Forms.DialogResult.No Then
                Exit Sub
            End If
        Else
            If objval.MsgConfirmYesNo("Are you sure you want to remove selected Items from Watchlist?", "Confirm") = Windows.Forms.DialogResult.No Then
                Exit Sub
            End If
        End If
        ...
        Dim cmd As New SQLiteCommand
        If AddStatus = True Then
            cmd.CommandText = "update db_products set is_watch=1 where product_id in (" & itemsStr & ")"
        Else
            cmd.CommandText = "update db_products set is_watch=0 where product_id in (" & itemsStr & ")"
        End If
...
rechek:
    ...
    Private Sub AwsMain_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        Me.Hide()
        Dim mn = New AwsSplashScreen
        mn.ShowDialog()
        Dim Login = New AWSLogin
        Login.ShowDialog()
        If (Login.DialogResult = Windows.Forms.DialogResult.OK) Then
            ...
            ReloadGrid()
            ScheduleTimer()
            'SendMail_SingleScan("B008PDAF7C", "TEST")
    ...
    Private Sub fill_LicDetails()
        bsi_UserName.Caption = "Current User : " & ProductLicDetails.CustEmail
        lbl_Validupto.Text = "Valid upto : " & ProductLicDetails.CustExpireDateDisplay
        If ProductLicDetails.CustExpireDateDisplay.StartsWith("Lic") Then
            lbl_Validupto.ForeColor = Color.Maroon
        Else
            lbl_Validupto.ForeColor = Color.ForestGreen
        End If
        lbl_License_Type.Text = "License Type : " & ProductLicDetails.CustUserTypeCode
    End Sub
    Private Sub SimpleButton3_Click(sender As Object, e As EventArgs) Handles btnSetAsActive.Click
        setActive(True)
    End Sub
    ...
    Private Sub btnRemoveFromWatchList_Click(sender As Object, e As EventArgs) Handles btnRemoveFromWatchList.Click
        setInWatchList(False)
    End Sub

    Private Sub btnSetAsInActive_Click(sender As Object, e As EventArgs) Handles btnSetAsInActive.Click
        setActive(False)
    End Sub

    Private Sub lbl_License_Main_Details_Click(sender As Object, e As EventArgs) Handles lbl_License_Type.Click

    End Sub

    Private Sub bbi_RegDetail_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbi_RegDetail.ItemClick
        Dim frm As New frmLicDetails
        frm.ShowDialog()
    End Sub

    Private Sub licCheckTimer_Tick(sender As Object, e As EventArgs) Handles licCheckTimer.Tick
        Dim dsResult As New DataSet
        dsResult = AwsCls.Check_LicenseValidation(ProductLicDetails.CustEmail, ProductLicDetails.CustTokenNo)
        ...
        Else
            MessageBox.Show("Unauthorised access found ! Application will be Stopped.", "Access", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Me.Close()
        End If
    End Sub
End Class
...
Public Class NotifierData
    Private privateIsWarning As Boolean
    Public Property IsWarning() As Boolean
        Get
            Return privateIsWarning
        End Get
        Set(ByVal value As Boolean)
            privateIsWarning = value
        End Set
    End Property
End Class